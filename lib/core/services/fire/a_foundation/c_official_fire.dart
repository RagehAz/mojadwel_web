...
abstract class OfficialFire{
  ...
  static Future<void> _setData({
    ...
    Function(String? error)? onError,
      ...
      final Map<String, dynamic>? _upload = cleanNullPairs(
        map: input,
      );
    ...
    await tryAndCatch(
        ...
        functions: () async {
          ...
          if (snapshot != null && snapshot.exists == true) {
            _output = getMapFromDocumentSnapshot(
      ...
      await tryAndCatch(
        ...
        functions: () async {
          ...
          if (query != null){
            ...
            _maps = getMapsFromQueryDocumentSnapshotsList(
    ...
    await tryAndCatch(
      ...
      functions: () async {
        ...
        if (_collRef != null) {
          ...
          _output = getMapsFromQueryDocumentSnapshotsList(
            queryDocumentSnapshots: _queryDocumentSnapshots,
            addDocsIDs: true,
  ...
  static Stream<List<Map<String, dynamic>>>? streamColl({
    ...
    return _query?.snapshots().map(mapSnapshots);
  }
  // --------------------
  /// TASK : TEST ME
  static Stream<Map<String, dynamic>?>? streamDoc({
    ...
    if (_stream == null){
      ...
      return _stream.map(mapSnapshot);
    }
  ...
  static Future<void> deleteColl({
    ...
    for (int i = 0; i < numberOfIterations; i++){
        ...
        if (_maps.isEmpty){
          ...
          final List<String> _docIDs = getMapsPrimaryKeysValues(
            maps: _maps,
            // primaryKey: 'id',
          );
