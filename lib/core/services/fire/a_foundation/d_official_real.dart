...
abstract class OfficialReal {
  ...
  static Future<void> _onRealError({
    ...
    // final bool _writeCancelled = stringContainsSubString(
    //   string: error,
    //   subString: 'write-cancelled',
    // );
    // final bool _writeCancelled2 = stringContainsSubString(
    //     string: error,
    ...
    if (isConnected == true){
      ...
      await tryAndCatch(
        ...
        functions: () async {
          ...
          if (snap != null) {
            _output = getMapsFromDataSnapshot(
              snapshot: snap,
              addDocID: true,
            )!;
          }
    ...
    if (isConnected == true){
      ...
      await tryAndCatch(
        ...
        functions: () async {
          ...
          if (_snap != null){
            _output = getMapFromDataSnapshot(
              snapshot: _snap,
              addDocID: true,
            );
          }
  ...
  static Future<Map<String, dynamic>?> updateDocInPath({
    ...
    if (isConnected == true){
      ...
      if (path != null && map != null){
        ...
        final String? _pathWithoutDocName = removeTextAfterLastSpecialCharacter(
          text: path,
          specialCharacter: '/',
        );
        final String? _docName = removeTextBeforeLastSpecialCharacter(
          text: path,
          specialCharacter: '/',
        );

        if (
            isEmpty(_pathWithoutDocName) == false
            &&
            isEmpty(_docName) == false
        ){
    ...
    if (isConnected == true){
      ...
      if (value != null && coll != null && doc != null && field != null){
        ...
        await tryAndCatch(
            ...
            onError: (String error) => _onRealError(
              error: error,
              path: fixPathFormatting('$coll/$doc/$field')!,
              invoker: 'OfficialReal.updateDocField',
            ),
            functions: () async {
  ...
  static Future<bool> incrementDocFields({
    ...
    if (isConnected == true){
      ...
      if (incrementationMap != null){
        ...
        final Map<String, dynamic>? _updatesMap = createPathValueMapFromIncrementationMap(
  ...
  static Future<bool> deletePath({
    ...
    if (isConnected == true){
      ...
      if (isEmpty(pathWithDocName) == false){
  ...
  static Future<Map<String, dynamic>> readOnlineUsers({
    ...
    return getMapFromIHLMOO(ihlmoo: _map) ?? {};
  }
  // --------------------------------------------------------------------------
}
